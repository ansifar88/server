...
export const Signup = async(req,res,next) => {
 ...
 export const login = async (req,res,next)=>{
    try {
        ...
        const token = jwt.sign({userId:user._id},process.env.JWTKEY, {expiresIn:"24hr"}) 
        console.log(token,"token");
        
        return res.status(200).json({access:true,token,user,message:"logged in"})
    } catch (error) {
    ...
    try {
        ...
        if (exist) {
            ...
            let user = await newUser.save().then(
                ...
                await User.updateOne({_id:user._id},{$set:{verified:true}})
                const token = jwt.sign({userId:user._id},process.env.JWTKEY,{expiresIn : "24hr"})
                return res.status(200).json({created:true,token:token,user,message: "Account Registered"})
            }
    } catch (error) {
        console.log(error.message);
    }
